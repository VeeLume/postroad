# packcore — working notes for Claude
...
- JDK 21. `./gradlew build` → `build/libs/packcore-<version>.jar`.
...
## Layout (`io.github.veelume.packcore`)

- `Packcore` — entry point; the only place that touches event buses.
- `PackcoreConfig` — common config; gameplay numbers the modpack overrides.
...
- `names/` — culture data (`data/packcore/cultures/*.json`) and the deterministic name generator.
...
- `command/PackcoreCommands` — `/packcore …`.
