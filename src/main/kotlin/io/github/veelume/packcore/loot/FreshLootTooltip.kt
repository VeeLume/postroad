...
object FreshLootTooltip {
    ...
    private fun onTooltip(event: ItemTooltipEvent) {
        ...
        if (fresh) {
            event.toolTip.add(
                Component.translatable("tooltip.packcore.fresh_loot", daysLeft).withStyle(ChatFormatting.GOLD),
            )
        }
        if (rate != null) {
            val key = if (fresh) "tooltip.packcore.buyback" else "tooltip.packcore.buyback_if_fresh"
