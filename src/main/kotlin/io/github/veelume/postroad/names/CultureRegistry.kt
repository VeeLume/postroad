package io.github.veelume.postroad.names
...
data class Culture(
    ...
    /** Courier-post palette style: `data/postroad/structure/courier_post_<post>.nbt`. */
    ...
    override fun apply(objects: Map<ResourceLocation, JsonElement>, manager: ResourceManager, profiler: ProfilerFiller) {
        ...
        for ((location, json) in objects) {
            try {
                ...
                Postroad.LOGGER.error("Skipping culture {}: {}", location, e.message)
            }
        }
        cultures = loaded
        Postroad.LOGGER.info("Loaded {} cultures", loaded.size)
    }
