...
object RoadPieceLayer {
    ...
    /** The flat band between two anchors four apart on both axes: a zigzag of 3×3 stamps, x first. */
    private fun layDiagonal(level: LevelAccessor, p: PiecePlacement, style: RoadStyle, styles: RoadStyleSet, groundAt: (Int, Int) -> Int, inArea: (Int, Int) -> Boolean, seed: Long): Int {
        ...
        val top = p.entry.y
        // Centres first so they own their block, then the rings.
        for (c in centres) done.add(key(c[0], c[1]))
        for (c in centres) {
            if (inArea(c[0], c[1])) placed += column(level, c[0], c[1], top, style.surface.pick(Random(Mth.getSeed(c[0], 0, c[1]) xor seed)), p.piece.fit, style, styles, groundAt(c[0], c[1]))
        }
        for (c in centres) for (oz in -1..1) for (ox in -1..1) {
            if (ox == 0 && oz == 0) continue
            val x = c[0] + ox; val z = c[1] + oz
            if (!done.add(key(x, z)) || !inArea(x, z)) continue
            placed += column(level, x, z, top, style.edge.pick(Random(Mth.getSeed(x, 0, z) xor seed)), p.piece.fit, style, styles, groundAt(x, z))
        }
        return placed
    }
